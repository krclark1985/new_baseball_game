...
from ..baseball_models import Team1Lineup, db

bp = Blueprint('team1lineup', __name__, url_prefix='/team1lineup')

# Read endpoint for index of all players in team 1 lineup table;
@bp.route('', methods=['GET']) # decorator takes path and list of HTTP verbs
def index():
    players = Team1Lineup.query.all() # ORM performs SELECT query
    result = []
    for p in players:
