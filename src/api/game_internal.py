...
def update_outs(current_game):
    if current_game.outs < 2:
        ...
        if current_game.top_of_inning == False:
            ...
            current_outcome.hit_outcome = f"{current_outcome} End of inning. {away_team} is now batting!"
        else:
            update_top_of_inning(current_game)
            update_reset(current_game)
            current_outcome = current_game.hit_outcome
            home_team = current_game.team2_name
            current_outcome.hit_outcome = f"{current_outcome} Three outs. {home_team} is now batting!"
